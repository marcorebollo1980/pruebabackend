...
public class PostSeeder {
    ...
    public void init() {
        if (postService.getAllPosts().isEmpty()) {
            ...
            postService.createPost(new com.redsocial.dto.CreatePostRequest(post1.getMessage()), post1.getUserId());

            Post post2 = new Post(null, "Disfrutando de un buen día.", 2L, LocalDateTime.now().minusHours(1), 0);
            postService.createPost(new com.redsocial.dto.CreatePostRequest(post2.getMessage()), post2.getUserId());

            Post post3 = new Post(null, "¡Qué interesante proyecto!", 1L, LocalDateTime.now(), 0);
            postService.createPost(new com.redsocial.dto.CreatePostRequest(post3.getMessage()), post3.getUserId());
        }
    }
}
